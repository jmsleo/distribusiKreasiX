...
class InvoicePDFGenerator:
    ...
    def generate_invoice(self, outlet_data, sales_data, invoice_number=None, invoice_date=None):
        ...
        # Invoice title
        invoice_title = Paragraph("INVOICE TAGIHAN", self.invoice_title_style)
        ...
        # Sales items table
        sales_header = Paragraph("RINCIAN PENJUALAN", self.section_header_style)
    ...
    def generate_outlet_invoice(self, data_helper, outlet_id, start_date=None, end_date=None):
        """Generate invoice for a specific outlet"""
        try:
            # Get outlet data
            ...
            if not sales_data:
                raise ValueError("Tidak ada data penjualan untuk outlet ini")
            ...
            return pdf_buffer, f"Invoice_{outlet_dict['nama'].replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.pdf"
